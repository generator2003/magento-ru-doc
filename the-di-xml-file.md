...
`Во время слияния аргументы заменяют другие аргументы с тем же именем, если их тип отличается. Если тип аргумента тот же, то новый аргумент заменяет старый.`

**Отображения абстракции-реализации**

Object manager использует абстрактную реализацию отображения когда 
Диспетчер объектов использует сопоставления абстракции-реализации, когда сигнатура конструктора класса запрашивает объект по его интерфейсу. Диспетчер объектов использует эти сопоставления, чтобы определить, какая реализация по умолчанию для этого класса для определенной области.

Preference node определяет реализацию по умолчанию:
```
<!--  File: app/etc/di.xml -->
<config>
...
```
Это сопоставление находится в app / etc / di.xml, поэтому диспетчер объектов вводит \ Core \ Model \ Url класс реализации везде, где есть запрос для Magento \Core\Model\ UrlInterface в глобальной области
